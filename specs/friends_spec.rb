...
class TestFriends < MiniTest::Test
  ...
  def test_fav_tv_show
    actual = fav_tv_show(@person1)
    assert_equal("Friends", actual)
  end
...
def test_is_fav_food_bread
  assert_equal(true, is_fav_food(@person2, "bread"))
end

def test_is_fav_food_pasta
  assert_equal(false, is_fav_food(@person2, "pasta"))
end
  ...
  def test_add_new_friend
    actual_number_of_friends = add_new_friend(@person3, @person4[:name]).length()
    assert_equal(3,actual_number_of_friends)
  end

  # 5. For a given person, remove a specific name from their list of friends
  # (hint: Same as above, testing for the length of the array should be sufficient)
  def test_removing_velma
    actual_number_of_friends = remove_new_friend(@person4, @person2[:name]).length()
    assert_equal(2,actual_number_of_friends)
  end
